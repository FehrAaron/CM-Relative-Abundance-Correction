...
#' Possible methods are c("satterthwaite", "no_df_approximation"). satterthwaite uses the Welch-Satterthwaite 
#' equation to estimate the degrees of freedom as described in https://doi.org/10.1016/j.mcpro.2022.100477. 
#' no_df_approximation just takes the number of peptides into account when calculating the degrees of freedom.
...
#' 
#' diff_lip = data %>%
#'     mutate(fg_intensity_log2 = log2(fg_quantity)) %>%
#'     assign_missingness(sample = r_file_name,
...
#'
#' diff_trp = data_trp %>%
#'      mutate(pg_intensity_log2 = log2(pg_quantity)) %>%
#'      assign_missingness(sample = r_file_name,
#'                    condition = r_condition,
...
#' 
#' corrected = correct_abundance_changes(
#' 
#'   lip_data = diff_lip,
#'   trp_data = diff_trp,
#'   comparison = comparison,
...
correct_abundance_changes = function(
  ...
  combined_data = dplyr::left_join(x = temp_lip_data,
                                   y = temp_trp_data,
